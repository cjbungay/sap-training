class CL_EX_BADI_BOOK20 definition
  public
  final
  create public .

*"* public components of class CL_EX_BADI_BOOK20
*"* do not include other source files here!!!
public section.

  interfaces IF_EX_BADI_BOOK20 .
  type-pools SEEX .
protected section.
*"* protected components of class CL_EX_BADI_BOOK20
*"* do not include other source files here!!!
private section.
*"* private components of class CL_EX_BADI_BOOK20
*"* do not include other source files here!!!

  data INSTANCE_BADI_TABLE type SEEX_EXIT_TAB .
ENDCLASS.



CLASS CL_EX_BADI_BOOK20 IMPLEMENTATION.


method IF_EX_BADI_BOOK20~CHANGE_VLINE.
CLASS CL_EXIT_MASTER DEFINITION LOAD.
DATA: EXIT_OBJ_TAB TYPE SEEX_EXIT_TAB,
      EXIT_OBJ TYPE SEEX_EXIT_TAB_STRUCT.

DATA: EXITINTF TYPE REF TO IF_EX_BADI_BOOK20.


  LOOP AT INSTANCE_BADI_TABLE INTO EXIT_OBJ WHERE
       INTER_NAME = 'IF_EX_BADI_BOOK20' AND
       FLT_VAL    = SPACE.
    APPEND EXIT_OBJ TO EXIT_OBJ_TAB.
  ENDLOOP.

  IF SY-SUBRC = 4.
    CALL METHOD CL_EXIT_MASTER=>CREATE_OBJ_BY_INTERFACE_FILTER
       EXPORTING
          INTER_NAME   = 'IF_EX_BADI_BOOK20'
          FLT_VAL      = SPACE
       IMPORTING
          EXIT_OBJ_TAB = EXIT_OBJ_TAB.

    APPEND LINES OF EXIT_OBJ_TAB TO INSTANCE_BADI_TABLE.
  ENDIF.

  LOOP AT EXIT_OBJ_TAB INTO EXIT_OBJ.

    CHECK NOT EXIT_OBJ-OBJ IS INITIAL.
    CHECK EXIT_OBJ-ACTIVE = SEEX_TRUE.


    CATCH SYSTEM-EXCEPTIONS MOVE_CAST_ERROR = 1.
      EXITINTF ?= EXIT_OBJ-OBJ.

      CALL FUNCTION 'PF_ASTAT_OPEN'
         EXPORTING
             OPENKEY = 'JR3n22}nr16hRm3Gjpo6nG'
             TYP     = 'UE'.

      CALL METHOD EXITINTF->CHANGE_VLINE

         CHANGING
           C_POS = C_POS.



      CALL FUNCTION 'PF_ASTAT_CLOSE'
         EXPORTING
             OPENKEY = 'JR3n22}nr16hRm3Gjpo6nG'
             TYP     = 'UE'.

    ENDCATCH.
  ENDLOOP.


ENDMETHOD.


method IF_EX_BADI_BOOK20~OUTPUT.
CLASS CL_EXIT_MASTER DEFINITION LOAD.
DATA: EXIT_OBJ_TAB TYPE SEEX_EXIT_TAB,
      EXIT_OBJ TYPE SEEX_EXIT_TAB_STRUCT.

DATA: EXITINTF TYPE REF TO IF_EX_BADI_BOOK20.


  LOOP AT INSTANCE_BADI_TABLE INTO EXIT_OBJ WHERE
       INTER_NAME = 'IF_EX_BADI_BOOK20' AND
       FLT_VAL    = SPACE.
    APPEND EXIT_OBJ TO EXIT_OBJ_TAB.
  ENDLOOP.

  IF SY-SUBRC = 4.
    CALL METHOD CL_EXIT_MASTER=>CREATE_OBJ_BY_INTERFACE_FILTER
       EXPORTING
          INTER_NAME   = 'IF_EX_BADI_BOOK20'
          FLT_VAL      = SPACE
       IMPORTING
          EXIT_OBJ_TAB = EXIT_OBJ_TAB.

    APPEND LINES OF EXIT_OBJ_TAB TO INSTANCE_BADI_TABLE.
  ENDIF.

  LOOP AT EXIT_OBJ_TAB INTO EXIT_OBJ.

    CHECK NOT EXIT_OBJ-OBJ IS INITIAL.
    CHECK EXIT_OBJ-ACTIVE = SEEX_TRUE.


    CATCH SYSTEM-EXCEPTIONS MOVE_CAST_ERROR = 1.
      EXITINTF ?= EXIT_OBJ-OBJ.

      CALL FUNCTION 'PF_ASTAT_OPEN'
         EXPORTING
             OPENKEY = 'KR3n22}nr16hRm3Gjpo6nG'
             TYP     = 'UE'.

      CALL METHOD EXITINTF->OUTPUT
         EXPORTING
           I_BOOKING = I_BOOKING.



      CALL FUNCTION 'PF_ASTAT_CLOSE'
         EXPORTING
             OPENKEY = 'KR3n22}nr16hRm3Gjpo6nG'
             TYP     = 'UE'.

    ENDCATCH.
  ENDLOOP.


ENDMETHOD.
ENDCLASS.
