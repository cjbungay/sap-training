...
class CL_EX_BADI_BOOK13 definition
  public
  final
  create public .

*" public components of class CL_EX_BADI_BOOK13
...
interfaces:
  IF_EX_BADI_BOOK13 .
protected section.
*" protected components of class CL_EX_BADI_BOOK13
*" do not include other source files here!!!

private section.
*" private components of class CL_EX_BADI_BOOK13
*" do not include other source files here!!!
...
CLASS CL_EX_BADI_BOOK13 IMPLEMENTATION.


method IF_EX_BADI_BOOK13~CHANGE_VLINE.
  class cl_exit_master definition load.
  data: EXIT_OBJ_TAB type SEEX_EXIT_TAB,
        EXIT_OBJ TYPE SEEX_EXIT_TAB_STRUCT.
  data exitintf type ref to IF_EX_BADI_BOOK13 .

  loop at EXIT_TABLE INTO EXIT_OBJ WHERE
          inter_name   = 'IF_EX_BADI_BOOK13' and
          FLT_VAL      = SPACE.
  ...
  loop at EXIT_OBJ_TAB into EXIT_OBJ.
    ...
    check EXIT_OBJ-ACTIVE = SEEX_TRUE.
      ...
      call function 'PF_ASTAT_OPEN'
        exporting
           OPENKEY = 'aCh{wYI2qn6AEm2WoSWnTW'
           TYP     = 'UE'.
...
method IF_EX_BADI_BOOK13~OUTPUT.
  class cl_exit_master definition load.
  data: EXIT_OBJ_TAB type SEEX_EXIT_TAB,
        EXIT_OBJ TYPE SEEX_EXIT_TAB_STRUCT.
  data exitintf type ref to IF_EX_BADI_BOOK13 .
  ...
  loop at EXIT_OBJ_TAB into EXIT_OBJ.
    ...
    check EXIT_OBJ-ACTIVE = SEEX_TRUE.
      ...
      call function 'PF_ASTAT_OPEN'
        exporting
           OPENKEY = 'Zyh{wYI2qn6AEm2WoSWnTW'
           TYP     = 'UE'.
