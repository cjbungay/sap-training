class CL_EX_BC425_16FLIGHT2 definition
  public
  final
  create public .

*"* public components of class CL_EX_BC425_16FLIGHT2
*"* do not include other source files here!!!
public section.

  interfaces IF_EX_BC425_16FLIGHT2 .

  class-data DYNPRO_INSTANCE type ref to IF_EX_BC425_16FLIGHT2
 read-only .
  constants VERSION type VERSION value 000001. "#EC NOTEXT

  class-methods GET_INSTANCE_FOR_SUBSCREEN
    returning
      value(INSTANCE) type ref to IF_EX_BC425_16FLIGHT2 .
  class-methods SET_INSTANCE_FOR_SUBSCREEN
    importing
      value(INSTANCE) type ref to IF_EX_BC425_16FLIGHT2 .
  type-pools SXRT .
protected section.
*"* protected components of class CL_EX_BC425_16FLIGHT2
*"* do not include other source files here!!!
private section.
*"* private components of class CL_EX_BC425_16FLIGHT2
*"* do not include other source files here!!!
...
CLASS CL_EX_BC425_16FLIGHT2 IMPLEMENTATION.


method GET_INSTANCE_FOR_SUBSCREEN .
  instance = CL_EX_BC425_16FLIGHT2=>DYNPRO_INSTANCE.
endmethod.


method IF_EX_BC425_16FLIGHT2~GET_DATA .
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB,
        old_imp_class type seoclsname.
  DATA: exitintf TYPE REF TO IF_EX_BC425_16FLIGHT2,
  ...
  IF sy-subrc NE 0.
    LOOP AT INSTANCE_BADI_TABLE ASSIGNING <exit_obj>
         WHERE INTER_NAME   = 'IF_EX_BC425_16FLIGHT2'
    ...
    IF sy-subrc = 4.
      CALL METHOD CL_EXIT_MASTER=>CREATE_OBJ_BY_INTERFACE_FILTER
         EXPORTING
            CALLER       = me
            INTER_NAME   = 'IF_EX_BC425_16FLIGHT2'
            METHOD_NAME  = 'GET_DATA'
  ...
  LOOP AT INSTANCE_FLT_CACHE ASSIGNING <flt_cache_line>
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'mvVIFL}F63hX00002X569m'
           TYP     = 'UE'.
        ...
        IF <flt_cache_line>-eo_object is initial.
          CALL METHOD ('CL_FOBU_METHOD_EVALUATION')=>load
               EXPORTING
                  ...
                  im_interface_name = 'IF_EX_BC425_16FLIGHT2'
...
method IF_EX_BC425_16FLIGHT2~PUT_DATA .
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB,
        old_imp_class type seoclsname.
  DATA: exitintf TYPE REF TO IF_EX_BC425_16FLIGHT2,
  ...
  LOOP AT INSTANCE_FLT_CACHE ASSIGNING <flt_cache_line>
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'oPVIFL}F63hX00002X569m'
           TYP     = 'UE'.
...
CL_EX_BC425_16FLIGHT2=>DYNPRO_INSTANCE = instance.
endmethod.
ENDCLASS.
