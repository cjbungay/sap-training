class CL_EX_BC427_EXT_FL_DISP definition
  public
  final
  create public .

*"* public components of class CL_EX_BC427_EXT_FL_DISP
*"* do not include other source files here!!!
public section.

  interfaces IF_EX_BC427_EXT_FL_DISP .

  constants VERSION type VERSION value 000001. "#EC NOTEXT
  type-pools SXRT .
protected section.
*"* protected components of class CL_EX_BC427_EXT_FL_DISP
*"* do not include other source files here!!!
private section.
*"* private components of class CL_EX_BC427_EXT_FL_DISP
*"* do not include other source files here!!!
...
CLASS CL_EX_BC427_EXT_FL_DISP IMPLEMENTATION.


method IF_EX_BC427_EXT_FL_DISP~WRITE_ADDITIONAL_COLS.
  CLASS CL_EXIT_MASTER DEFINITION LOAD.
  DATA: EXIT_OBJ_TAB TYPE SXRT_EXIT_TAB.

  DATA: exitintf TYPE REF TO IF_EX_BC427_EXT_FL_DISP,
    ...
    <flt_cache_line>-method_name = 'WRITE_ADDITIONAL_COLS'.
      ...
      IF sy-subrc ne 0.
        CALL METHOD CL_EXIT_MASTER=>CREATE_OBJ_BY_INTERFACE_FILTER
           EXPORTING
              CALLER       = me
              INTER_NAME   = 'IF_EX_BC427_EXT_FL_DISP'
              METHOD_NAME  = 'WRITE_ADDITIONAL_COLS'
  ...
  LOOP AT INSTANCE_FLT_CACHE ASSIGNING <flt_cache_line>
    ...
    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'Hgz6lvjyH2RX00002a92EG'
           TYP     = 'UE'.
        ...
        IF <flt_cache_line>-eo_object is initial.
          CALL METHOD ('CL_FOBU_METHOD_EVALUATION')=>load
               EXPORTING
                  ...
                  im_interface_name = 'IF_EX_BC427_EXT_FL_DISP'
