*&---------------------------------------------------------------------*
*& Report  SAPBC412_CFWD_EVENTS_C                                      *
*&                                                                     *
*&---------------------------------------------------------------------*
*& Sixth demo program of a series of demo programs of course BC412     *
...
*&     - NAVIGATE_COMPLETE   as application event                      *
*&     - SAPEVENT            as system event                           *
*& This demo focusses on the 'problem' of how to select the order of
*& field transport (connected to field input check) and transfer
*& of control to the control framework for the handling of application
*& events.
*& ...
*&---------------------------------------------------------------------*

REPORT  sapbc412_cfwd_events_c MESSAGE-ID bc412.
...
DATA:
    ...
    input_field(5) TYPE c,             " dummy screen field
...
PARAMETERS: p_behind AS CHECKBOX.
...
*&---------------------------------------------------------------------*
*&      Module  NAVIGATE  INPUT
*&---------------------------------------------------------------------*
*       This module is executed every time the has entered a new
*       URL in the screen.
*----------------------------------------------------------------------*
MODULE navigate INPUT.
...
ENDMODULE.                             " NAVIGATE  INPUT
*&---------------------------------------------------------------------*
*&      Module  EXECUTE_APPLICATION_EVENTS  INPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
MODULE execute_application_events INPUT.
  IF p_behind = 'X'.
*   here no control to cfw, since the module is called before the
*   modules that check user input
  ELSE.
    CALL METHOD cl_gui_cfw=>dispatch.
  ENDIF.
ENDMODULE.                 " EXECUTE_APPLICATION_EVENTS  INPUT
*&---------------------------------------------------------------------*
*&      Module  CHECK_USER_INPUT  INPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
MODULE check_user_input INPUT.
  IF NOT input_field IS INITIAL.
    MESSAGE e022(bc412).
  ENDIF.
ENDMODULE.                             " CHECK_USER_INPUT  INPUT
