*&---------------------------------------------------------------------*
*& SAPBC412_TRMD_LIST_1                                                *
*&                                                                     *
*&---------------------------------------------------------------------*
*&                                                                     *
*& The program displays SCARR, SPFLI and SFLIGHT data                  *
*& using an List Tree Model.                                           *
*&                                                                     *
*&---------------------------------------------------------------------*

REPORT  sapbc412_trmd_list_1 MESSAGE-ID bc412.
...
* content:
  ref_tree_model TYPE REF TO cl_list_tree_model,
...
*  -->  L_REF_TREE_MODEL                                              *
*---------------------------------------------------------------------*
FORM add_nodes USING l_ref_tree_model TYPE REF TO cl_list_tree_model.

  DATA:
    l_wa_node  TYPE treemlnodt,
    l_it_items TYPE treemlitab,
    l_wa_item  LIKE LINE OF l_it_items,
...
* fill item table for the root node:
  CLEAR l_it_items.
  l_wa_item-class     = cl_list_tree_model=>item_class_text.
  l_wa_item-alignment = cl_list_tree_model=>align_auto.

  l_wa_item-item_name = '1'.
  l_wa_item-text      = text-car.
  INSERT l_wa_item INTO TABLE l_it_items.
...
*   fill item table for this node:
    CLEAR l_it_items.
    l_wa_item-class     = cl_list_tree_model=>item_class_text.
    l_wa_item-alignment = cl_list_tree_model=>align_auto.

    l_wa_item-item_name = '1'.
    l_wa_item-text      = wa_scarr-carrname.
    INSERT l_wa_item INTO TABLE l_it_items.

    l_wa_item-item_name = '2'.
    l_wa_item-text      = wa_scarr-currcode.
    INSERT l_wa_item INTO TABLE l_it_items.
    ...
    CALL METHOD l_ref_tree_model->add_node
      EXPORTING
        ...
        isfolder          = 'X'
        expander          = space
        last_hitem        = '2' "carrname, currency
        item_table        = l_it_items
      EXCEPTIONS
        OTHERS            = 5
            .
...
*   fill item table for this node:
    ...
    l_wa_item-item_name = '1'.
    l_wa_item-text      = l_conn_text.
    INSERT l_wa_item INTO TABLE l_it_items.

    l_wa_item-item_name = '2'.
    IF wa_spfli-fltype = 'X'.
      l_wa_item-text      = text-cha.
    ELSE.
    ...
    CALL METHOD l_ref_tree_model->add_node
      EXPORTING
        ...
        isfolder          = 'X'
        expander          = space
        last_hitem        = '2' "connection, charter y/n
        item_table        = l_it_items
      EXCEPTIONS
        OTHERS            = 5
        .
...
*   fill item table for this node:
    ...
    l_wa_item-item_name = '1'.
    l_wa_item-text      = l_date_text.
    INSERT l_wa_item INTO TABLE l_it_items.

    l_wa_item-item_name = '2'.
    l_wa_item-text      = l_price_text.
    INSERT l_wa_item INTO TABLE l_it_items.

    l_wa_item-item_name = '3'.
    l_wa_item-text      = wa_sflight-currency.
    INSERT l_wa_item INTO TABLE l_it_items.

    l_wa_item-item_name = '4'.
    l_wa_item-text      = wa_sflight-planetype.
    INSERT l_wa_item INTO TABLE l_it_items.

    l_wa_item-item_name = '5'.
    l_wa_item-text      = l_seats_text.
    INSERT l_wa_item INTO TABLE l_it_items.
    ...
    CALL METHOD l_ref_tree_model->add_node
      EXPORTING
        ...
        isfolder          = space
        expander          = space
        last_hitem        = '5' "fldate, price, currency,
                                "planetype, free seats
        item_table        = l_it_items
      EXCEPTIONS
        OTHERS            = 5
        .
