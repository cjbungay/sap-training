...
CONSTANTS:
  c_template_form TYPE fpwbformname VALUE 'BC480_LAYOUT_TEST'.
...
FORM add_layout .
  DATA:
     ls_fpcontext type fpcontext,
     ls_fplayout TYPE fplayout,
     ls_dwinactiv TYPE dwinactiv.

* make sure inactive context exists to enforce activation
  SELECT SINGLE *
    FROM fpcontext
    INTO ls_fpcontext
    WHERE name = pa_form
  ...
  IF sy-subrc <> 0.
    ...
    WHERE name = pa_form
    AND state = 'A'.
    ls_fpcontext-state = 'I'.
    INSERT fpcontext FROM ls_fpcontext.
  ENDIF.

* make sure inactive layout exists to enforce activation
  SELECT SINGLE *
    FROM fplayout
    INTO ls_fplayout
    WHERE name = pa_form
  ...
  IF sy-subrc <> 0.
    ...
    AND   state = 'A'.
    ls_fplayout-state = 'I'.
    INSERT fplayout FROM ls_fplayout.
  ENDIF.

* make sure form is considered inactive
  ls_dwinactiv-object = 'SFPF'.
  ls_dwinactiv-obj_name = pa_form.
  ls_dwinactiv-uname = sy-uname.
  ls_dwinactiv-delet_flag = space.

  MODIFY dwinactiv FROM ls_dwinactiv.
