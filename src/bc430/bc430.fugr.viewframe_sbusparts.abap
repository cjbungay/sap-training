*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_SBUSPARTS
*   generation date: 19.11.1997 at 16:15:23 by user SCHREPPM
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_SBUSPARTS           .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_SBUSPARTS'
         EXPORTING
              FCODE          = READ
...
EXTRACT = SBUSPARTS_EXTRACT
TOTAL = SBUSPARTS_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_SBUSPARTS'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_SBUSPARTS-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = SBUSPARTS_EXTRACT
TOTAL = SBUSPARTS_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_SBUSPARTS-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_SBUSPARTS'
...
TOTAL = SBUSPARTS_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_SBUSPARTS-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_SBUSPARTS-UPD_FLAG NE SPACE.
