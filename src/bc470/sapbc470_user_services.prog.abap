*----------------------------------------------------------------------*
*  This report could be run prior to BC470 classes.
...
*
*----------------------------------------------------------------------*
REPORT  sapbc470_user_services MESSAGE-ID bc470.
TYPE-POOLS: trdev.
...
*----------------------------------------------------------------------*
PARAMETERS:
  pa_cours TYPE bctools_log-course DEFAULT 'BC470' OBLIGATORY.
SELECT-OPTIONS:
  so_count FOR diff NO-EXTENSION DEFAULT 0 TO 18.
SELECT-OPTIONS:
  so_user  FOR usr01-bname DEFAULT 'BC470-00' TO 'BC470-18'
...
SELECTION-SCREEN COMMENT 1(25) text-s01 FOR FIELD pa_trkor.
...
SELECTION-SCREEN BEGIN OF BLOCK action WITH FRAME TITLE text-s05.

* 1st part: create package for every course participant
PARAMETERS:
  pa_pack AS CHECKBOX DEFAULT 'X' USER-COMMAND devc.
SELECT-OPTIONS:
  so_devc  FOR devclass DEFAULT 'ZBC470_00' TO 'ZBC470_18'
    NO-EXTENSION MODIF ID inf.

SELECTION-SCREEN SKIP.

* 2nd part: set default printer in user settings
SELECTION-SCREEN BEGIN OF BLOCK prnt WITH FRAME TITLE text-s03.
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS:
  pa_defpr AS CHECKBOX DEFAULT 'X'.
SELECTION-SCREEN COMMENT 4(25) text-s04 FOR FIELD pa_defpr.
SELECTION-SCREEN POSITION POS_LOW.
PARAMETERS:
...
* set the transport layer on the screen.
    ...
    SELECT SINGLE translayer                            "#EC CI_NOFIRST
      FROM vtcetral
      INTO pa_layer
...
AT SELECTION-SCREEN.
  ...
  IF pa_pack IS NOT INITIAL.
    SELECT devclass                                   "#EC CI_SGLSELECT
      FROM tdevc
      INTO devclass
  ...
  IF pa_pack IS INITIAL AND pa_defpr IS INITIAL AND pa_immed IS INITIAL.
...
* 3rd part: disallow immediate printing for course participants
    ...
    rseumod-progname = 'SAPBC470_DEMO'.
    MODIFY rseumod.
  ENDLOOP.
