...
CLASS lcl_bus DEFINITION INHERITING FROM lcl_vehicle.
  ...
  PUBLIC SECTION.
    ...
    METHODS       display_attributes REDEFINITION.
    METHODS:      get_passengers RETURNING value(re_pass) TYPE i.
  PRIVATE SECTION.
    "-------------------
...
CLASS lcl_bus IMPLEMENTATION.
  METHOD get_passengers.
    re_pass = max_passengers.
  ENDMETHOD.                    "get_passengers
...
*        r_bus->get_passengers( ).
  ...
  METHOD  display_attributes.
    DATA: r_vehicle TYPE REF TO lcl_vehicle.
    data: r_bus TYPE REF TO lcl_bus,
          pass TYPE i.
...
*    using caught widening cast: ref_bus ?= sender)
    ...
    LOOP AT vehicle_list INTO r_vehicle.
      ...
      ref_class_descr ?=
        cl_abap_typedescr=>describe_by_object_ref( r_vehicle ).

      IF ref_class_descr->get_relative_name( ) = 'LCL_BUS'.
        r_bus ?= r_vehicle.
        pass = r_bus->get_passengers( ).
        write: / 'PASS = ', pass.
      endif.
