class CB_BC401_POJD_CARRIER definition
  public
  inheriting from CL_OS_CA_COMMON
  abstract
  create public .

*"* public components of class CB_BC401_POJD_CARRIER
...
public section.
  ...
  methods CREATE_PERSISTENT
    importing
      ...
      !I_CARRNAME type S_CARRNAME optional
      !I_CURRCODE type S_CURRCODE optional
      !I_URL type S_CARRURL optional
    returning
      value(RESULT) type ref to CL_BC401_POJD_CARRIER
    raising
  ...
  methods IF_OS_FACTORY~CREATE_TRANSIENT_BY_KEY
    redefinition .
protected section.
*"* protected components of class CB_BC401_POJD_CARRIER
*"* do not include other source files here!!!

  types TYP_OID type OS_GUID .
  types TYP_TYPE type OS_GUID .
  types TYP_OBJECT_REF type ref to CL_BC401_POJD_CARRIER .
  types:
  ...
  methods SAVE_SPECIAL_OBJECT_INFO
    redefinition .
private section.
*"* private components of class CB_BC401_POJD_CARRIER
*"* do not include other source files here!!!
ENDCLASS.



CLASS CB_BC401_POJD_CARRIER IMPLEMENTATION.


method CREATE_PERSISTENT .
***BUILD 020501
*      IMPORTING I_CARRID TYPE S_CARR_ID
*      IMPORTING I_CARRNAME TYPE S_CARRNAME
*      IMPORTING I_CURRCODE TYPE S_CURRCODE
*      IMPORTING I_URL TYPE S_CARRURL
*      RETURNING RESULT TYPE REF TO CL_BC401_POJD_CARRIER
************************************************************************
...
*                  closed.
*
* OO Exceptions  : changes CX_OS_OBJECT_EXISTING(
*                            PERSISTENT_CREATING_PERSISTENT,
*                            TRANSIENT_CREATING_PERSISTENT)
*                    from OS_PM_HANDLE_CREATE_ON_EXIST
...
*< Generated from mapping:
  data: THE_OBJECT type ref to CL_BC401_POJD_CARRIER.
*>
...
*   * To be on the safe side: Set the object's attributes
*<  Generated from mapping:
...
*   * 4. Set attributes
*<   Generated from mapping:
    THE_OBJECT->CARRNAME = I_CARRNAME.
    THE_OBJECT->CURRCODE = I_CURRCODE.
    THE_OBJECT->URL = I_URL.
*>
...
* Implementation : 1. Check if there is already an object with the
*                     same key
*                  2. If there is one, raise an exception
*                  3. If there is none, create a new object
*                  4. Set Attributes
...
*   * 2. If there is one, raise an exception
*!! error: object with specified key already exists
...
* *    NEW, CHANGED or DELETED
  ...
  loop at OBJECT_INFO assigning <FS_OBJECT_INFO_ITEM>
       where ( PM_STATUS = OSCON_OSTATUS_NEW or
               PM_STATUS = OSCON_OSTATUS_CHANGED or
               PM_STATUS = OSCON_OSTATUS_DELETED ) and
               OM_IGNORE = OSCON_FALSE.
...
***BUILD 020501
     ...
     " exporting E_OBJECT_DATA_TAB type TYP_DB_DATA_TAB
************************************************************************
...
************************************************************************
  ...
  data: THE_OBJECT type ref to CL_BC401_POJD_CARRIER,
        ...
        PM_SERVICE       type ref to IF_OS_PM_SERVICE.
...
************************************************************************
  ...
  data: OBJECT_IREF type TYP_OBJECT_IREF,
        THE_OBJECT  type ref TO CL_BC401_POJD_CARRIER.
...
************************************************************************
  ...
  data: DB_BUSINESS_KEY type TYP_BUSINESS_KEY ,
        DB_DATA         type TYP_DB_DATA .

*< Generated from mapping:
  data: DB_ITEM_SCARR type SCARR ,
        DB_TAB_SCARR  type standard table of SCARR .
*>

* * Load more than one entry?
  if ( lines( I_BUSINESS_KEY_TAB ) = 1 ).

*   * Load exactly one entry
    read table I_BUSINESS_KEY_TAB
         into DB_BUSINESS_KEY
         index 1.

*< Generated from mapping:
    select single * from SCARR into DB_ITEM_SCARR
           where CARRID = DB_BUSINESS_KEY-CARRID.
*>

*   * error handling
    if ( SY-SUBRC <> 0 ).
      BKEY_STRING = MAP_SERIALIZE_BKEY( DB_BUSINESS_KEY ).
      class CX_OS_DB_SELECT definition load.
      raise exception type CX_OS_DB_SELECT
           exporting BKEY   = BKEY_STRING
                     TABLE  = 'SCARR'
                     TEXTID = CX_OS_DB_SELECT=>BY_BKEY.
    endif. "( Error )

*< Generated from mapping:
    DB_DATA-CARRID = DB_ITEM_SCARR-CARRID.
    DB_DATA-CARRNAME = DB_ITEM_SCARR-CARRNAME.
    DB_DATA-CURRCODE = DB_ITEM_SCARR-CURRCODE.
    DB_DATA-URL = DB_ITEM_SCARR-URL.
*>
    append DB_DATA to RESULT.

  else. "( more than one entry to load )

    data: BUSINESS_KEY_TAB type TYP_BUSINESS_KEY_TAB.

*< Generated from mapping:
    field-symbols <FS_DB_SCARR> type SCARR.
*>

    BUSINESS_KEY_TAB = I_BUSINESS_KEY_TAB.

*< Generated from mapping:
    select * from SCARR into table DB_TAB_SCARR
           for all entries in BUSINESS_KEY_TAB
           where CARRID = BUSINESS_KEY_TAB-CARRID.
*>
...
*   * error handling
    ...
    loop at DB_TAB_SCARR assigning <FS_DB_SCARR>.
...
*>
  ...
  endif. "( one or more entries to load? )
...
*< Generated from mapping:
   DB_ILN_SCARR-CARRNAME = <FS_INSERT>-CARRNAME.   DB_ILN_SCARR-CURRCODE
   = <FS_INSERT>-CURRCODE.   DB_ILN_SCARR-URL = <FS_INSERT>-URL.
...
*< Generated from mapping:
   DB_ULN_SCARR-CARRNAME = <FS_UPDATE>-CARRNAME.   DB_ULN_SCARR-CURRCODE
   = <FS_UPDATE>-CURRCODE.   DB_ULN_SCARR-URL = <FS_UPDATE>-URL.
...
************************************************************************
  ...
  data: OBJECT_DATA_TAB     type TYP_DB_DATA_TAB,
        ...
        BUSINESS_KEY        type TYP_BUSINESS_KEY,
        LOADED_BUSINESS_KEY type TYP_BUSINESS_KEY,
        BUSINESS_KEY_TAB    type TYP_BUSINESS_KEY_TAB,
        TEMP_CURRENT_OBJECT_INFO  type TYP_OBJECT_INFO,
...
*       * A loaded object can be set again
...
*!!!!!!!!! Error: Object is transient
      ...
      endcase. "( State differs from "not loaded" )
...
************************************************************************
  ...
  data: NEW_OBJECT type ref to CL_BC401_POJD_CARRIER.
...
************************************************************************
  ...
  data: SPECIAL_OBJECT_INFO_ITEM type TYP_SPECIAL_OBJECT_INFO ,
        BUSINESS_KEY_TAB         type TYP_BUSINESS_KEY_TAB ,
        OBJECT_DATA_TAB          type TYP_DB_DATA_TAB ,
        OBJECT_REF_TAB           type TYP_OBJECT_REF_TAB ,
        OBJECT_TAB               type TYP_OBJECT_TAB ,
...
************************************************************************
 ...
 data: ex_os_db_select type ref to cx_os_db_select.
