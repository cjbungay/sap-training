class CL_EX_BADI_BC401 definition
  public
  final
  create public .

*"* public components of class CL_EX_BADI_BC401
...
public section.
  ...
  type-pools SXRT .
protected section.
*"* protected components of class CL_EX_BADI_BC401
*"* do not include other source files here!!!
private section.
*"* private components of class CL_EX_BADI_BC401
*"* do not include other source files here!!!
...
CLASS CL_EX_BADI_BC401 IMPLEMENTATION.
...
method IF_EX_BADI_BC401~ENHANCE_LIST.
  ...
  DATA: exitintf TYPE REF TO IF_EX_BADI_BC401,
  ...
  IF sy-subrc NE 0.
    ...
    <flt_cache_line>-method_name = 'ENHANCE_LIST'.


      LOOP AT INSTANCE_BADI_TABLE ASSIGNING <exit_obj>
           WHERE METHOD_NAME  = 'ENHANCE_LIST'.
  ...
  LOOP AT INSTANCE_FLT_CACHE ASSIGNING <flt_cache_line>
       WHERE flt_name    = flt_name
         AND valid       = sxrt_true
         AND method_name = 'ENHANCE_LIST'.


    CALL FUNCTION 'PF_ASTAT_OPEN'
       EXPORTING
           OPENKEY = 'WfhQGQ63PYhX00002X57DG'
           TYP     = 'UE'.
    ...
    CASE <flt_cache_line>-imp_switch.
      WHEN 'VSR'.
        ...
        IF <flt_cache_line>-eo_object is initial.
          CALL METHOD ('CL_FOBU_METHOD_EVALUATION')=>load
               EXPORTING
                  ...
                  im_interface_name = 'IF_EX_BADI_BC401'
                  im_method_name    = 'ENHANCE_LIST'
               RECEIVING
        ...
        CLEAR data_ref.
        GET REFERENCE OF IM_R_AGENCY INTO data_ref.
        CALL METHOD <flt_cache_line>-eo_object->set_parameter(
            im_parmname = 'IM_R_AGENCY'
            im_value    = data_ref ).
      ...
      WHEN OTHERS.
        ...
        CALL METHOD EXITINTF->ENHANCE_LIST
           EXPORTING
             IM_R_AGENCY = IM_R_AGENCY.
