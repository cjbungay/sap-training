REPORT sapbc420_sptd_var_format_bin MESSAGE-ID bc420.
*
* reads debitors and bankdata, both in two different records A and B.
* B contains bankdata. The inputfile is in binary mode.
*
TYPES:
******************* legacy structure A *****************************
   BEGIN OF reca,
...
*##################################
  ...
  CONCATENATE path 'BC420_DEBI_BANK_BIN.LEG' INTO infile.
  CONCATENATE path 'BC420_DEBI_BANK_BIN.SAP' INTO outfile.
  CONCATENATE path 'BC420_DEBI_BANK.ERR' INTO errfile.

AT SELECTION-SCREEN.
*#######################
* open files
  OPEN DATASET: infile FOR INPUT IN BINARY MODE MESSAGE text.
  IF sy-subrc NE 0.
    WRITE: / infile.
