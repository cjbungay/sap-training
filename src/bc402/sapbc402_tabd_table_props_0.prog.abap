*&---------------------------------------------------------------------*
*& Report  SAPBC402_TABD_TABLE_PROPS_0                                 *
*&                                                                     *
*&---------------------------------------------------------------------*
*& BC402 demonstration                                                 *
*& Scenario:                                                           *
...
*&     system                                                          *
*& This demo consists of four steps. Each step (program) shows a new   *
*& aspect with respect to the above objectives:                        *
*&   Step 0: implementation without using special table kinds or       *
*&           table operation properties                                *
*&   Step 1: use for IT_RESULT a table type with a unique key! So      *
*&           elimination of duplicates can be dropped!                 *
*&           HASHED or SORTED table kind are possible, step 1 uses     *
*&           a HASHED table.                                           *
*&   Step 2: use performance optimization of SORTED tables for         *
*&           IT_CONNECTIONS: the inner LOOP is a loop where the first  *
*&           key field of the table key is determined uniquely.        *
*&   Step 3: Use copy-free loops via LOOP ... ASSIGNING                *
*&                                                                     *
*&   ----------------------------------------------------------------- *
*&   The current demo is the program displaying STEP 0.                *
*&   ----------------------------------------------------------------- *
*&   STANDARD tables are used. To optimize the inner LOOP:             *
*&   1. IT_CONNECTIONS is sorted by airpfrom                           *
*&   2. The start index is determine via READ                          *
*&   3. The inner loop is exit via a check of a logical condition      *
*&   The duplicates are eliminated using DELETE ADJACENT ...           *
*&                                                                     *
*&---------------------------------------------------------------------*
